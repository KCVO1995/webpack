...
writeFileSync('loader/dist.js', generateCode())
console.log('done')
...
function generateCode() {
  ...
  code += 'var depRelation = [' + depRelation.map(item => {
    ...
    return `{
      key: ${JSON.stringify(key)}, // JSON.stringify 的作用是在两边加上引号
...
function collectCodeAndDeps(filepath: string) {
  ...
  if (/\.css$/.test(filepath)) { // 加载了 css 文件
    code = require('./loaders_2/css-loader')(code)
    code = require('./loaders_2/style-loader')(code)
  }
